...
if __name__== "__main__":
    ...
    root_out_dir = '/group/30098/zhouxiawang/outputs/MotionCtrlV2/tmp320sparseV2_21_len4'
    root_out_dir = './examples/webvid/dense_traj'
    os.makedirs(root_out_dir, exist_ok=True)

    for j, batch in tqdm(enumerate(dataloader), desc="Data Batch"):
        print(f"video={batch['video'].shape}")
        print(batch['path'])
        out_dir = f'{root_out_dir}/{batch["path"][0].split("/")[-1]}'
        os.makedirs(out_dir, exist_ok=True)

        np.save(f'{out_dir}/trajs.npy', batch['trajs'][0].numpy())
        np.save(f'{out_dir}/video.npy', batch['video'][0].permute(1,2,3,0).numpy())
        with open(f'{out_dir}/caption.txt', 'w') as f:
            f.write(batch['caption'][0]+'\n')
            f.write(batch['path'][0])
        ...
        filename = f'{out_dir}/video'
        
        video = []
        for i in range(1, frames.shape[0]):
            flow_img = torch.Tensor(frames[i]) # 256 x 256 x 3
        ...
        filename = f'{out_dir}/optical_flow'
        optical_flow = batch['trajs'][0].permute(1,2,3,0).numpy()
        print(len(np.where(optical_flow == 0)[0]))
        # import pdb; pdb.set_trace()
        video = []
        for i in range(1, optical_flow.shape[0]):
